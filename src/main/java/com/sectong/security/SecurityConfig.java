	...
	public static class FormLoginWebSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
		...
		protected void configure(HttpSecurity http) throws Exception {
            ...
            http.authorizeRequests()
                    .antMatchers("/api/create", "/api/forgottenPwd", "/api/verificationCode/**", "/", "/assets/**", "/plugins/**", "/static/**", "/bootstrap/**",
                            "/api-docs/**", "/debug/**", "/api/**", "/weatherstation/**", "/mac/**", "/v/**", "/red/**","/swagger-ui/**") // 免认证目录
                    .permitAll().antMatchers("/admin/**").hasRole("ADMIN")// ADMIN角色可以访问/admin目录
                    .anyRequest()
                    .authenticated().and().formLogin().loginPage("/login")// 自定义登录页为/login
